 ...
 */
class InTranPhysicalToolTest extends PHPUnit2_Framework_TestCase {
	...
	public function _testPhysical($tran) {
		$inTranBo=new CalemInTranBo();
		$rtn=$inTranBo->physical($tran);		
		//Add an item for this test.
		if (!isset($_ENV['CALEM_BATCH_TEST'])) {
			echo "<b>checkin result</b>=" . var_export($rtn, true) . "<br>";
		}
		//Verify at in level
		...
		//Verify stock
		$this->assertTrue($inRow['qty_in_stock']==$this->inAr['qty_in_stock']-$this->inStockAr1['qty']+$this->receiveTran['qty']);
		...
		$this->assertTrue($stockRow['qty']==$this->receiveTran['qty']);
	...
	//Checkout to asset	 
	public function testAvgPhysical() {
		$this->cleanupData();
		$this->setupData();
		$tran=$this->receiveTran;
		$tran['in_id']=$this->inAr['id'];
		//Run checkout
		$this->_testPhysical($tran);		
		//cleanup
		$this->cleanupData();                 
	}
}
...
if (!isset($_ENV['CALEM_BATCH_TEST'])) {
	$res=new InTranPhysicalToolTest();
	$res->testAvgPhysical();
}
?>
