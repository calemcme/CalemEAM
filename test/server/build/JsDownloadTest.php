...
class JsDownloadTest extends PHPUnit2_Framework_TestCase {
	...
	public function loadJs($name, $mode, $store=false) {
		global $_CALEM_conf;
		$endpoint = 'localhost' . $_CALEM_conf['calem_root_uri'] . '/client/JsPkg.php?loadmode='.$mode . '&js=' . $name;
		$query="";
		$response=$this->post($endpoint, 80, $query);
		if (!isset($_ENV['CALEM_BATCH_TEST'])) {
			echo $name . ".".$mode." file length=".strlen($response) . "<br>\n";
		}
		if ($store) {
	...
	public function testAjaxLoading() {
		$aggrSize=$this->loadJs('Ajax', 'aggr');
		$minSize=$this->loadJs('Ajax', 'min');
		$zipSize=$this->loadJs('Ajax', 'gzip');
		$this->assertTrue($aggrSize > $minSize && $minSize>$zipSize);
	}
	
	public function testCalemLoading() {
		$aggrSize=$this->loadJs('Calem', 'aggr');
		$minSize=$this->loadJs('Calem', 'min');
		$zipSize=$this->loadJs('Calem', 'gzip');
		$this->assertTrue($aggrSize > $minSize && $minSize>$zipSize);
	}
}

if (!isset($_ENV['CALEM_BATCH_TEST'])) {
	$res=new JsDownloadTest();
	$res->testAjaxLoading();
	$res->testCalemLoading();
}
?>
