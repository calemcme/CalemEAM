 ...
 */
class InTranUserToolTest extends PHPUnit2_Framework_TestCase {
	...
	public function testAvgCheckout() {
		...
		$tran=array(
			...
			'location_id'=>'10001',
			'qty'=>2,
			'checkout_type_id'=>'ict_user',
			'checkout_to_id'=>'user-001',
			'note'=>'test1',
	...
	public function testAvgReturn() {
		...
		//Run return
		$inTranDbo=CalemFactory::getDbo('in_tran');
		$rows=$inTranDbo->fetchBySqlParam('select id from in_tran where in_id=? and checkout_to_id=?', array($this->inAr['id'], 'user-001'));
