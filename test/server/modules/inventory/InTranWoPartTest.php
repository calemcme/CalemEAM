 ...
 */
class InTranWoPartTest extends PHPUnit2_Framework_TestCase {
	...
	public function cleanupData() {
		...
		$dbo->deleteBySqlParam('delete from wo_part where wo_id=?', $this->woAr['id']);
		//Ensure all is done.
		...
		$this->assertTrue(0==$dbo->getcountBySqlParam('select count(*) from wo_part where wo_id=?', $woId));	
	}
	...
	public function _testCheckout($tran) {
		...
		$this->assertTrue($assetRow['maint_material_cost']==$this->assetAr['maint_material_cost']+2*$inRow['avg_unit_cost']);
		...
		$this->assertTrue($row['material_cost'] == $this->woAr['material_cost'] + 2*$inRow['avg_unit_cost']);
		$this->assertTrue($row['total_cost'] == $this->woAr['material_cost'] + $this->woAr['labor_cost'] + 2*$inRow['avg_unit_cost']);
		//Verify wop
		$wopDbo=CalemFactory::getDbo('wo_part');
		$rows=$wopDbo->fetchBySqlParam('select * from wo_part where wo_id=? and in_id=?', array($this->woAr['id'], $inRow['id']));
		$row=$rows[0];
		if (!isset($_ENV['CALEM_BATCH_TEST'])) {
			echo 'wo part=' . var_export($row, true) . "<br>";
		} 			
		$this->assertTrue($row['qty_used']==2 && $row['line_cost']==2*$inRow['avg_unit_cost']);
		//Try out reserved
		if ($this->woAr['status_id'] != 'wos_closed') {
			...
			$this->assertTrue($row['qty']==$this->inReservedAr['qty']-2);
		}			        
	}
	...
	public function _testReturn($tran, $tranReturn) {
		...
		if ($this->woAr['status_id']=='wos_closed') {
			...
			if (!isset($_ENV['CALEM_BATCH_TEST'])) {
				echo "wo not closed, no need to verify asset cost adjustment.<br>";
			} 
		}
		//Verify workorder tables
		$woDbo=CalemFactory::getDbo('workorder');
		...
		$this->assertTrue($row['material_cost'] == $this->woAr['material_cost'] + 2*$inRow['avg_unit_cost']-$rtn['tran_total']);
		$this->assertTrue($row['total_cost'] == $this->woAr['material_cost'] + $this->woAr['labor_cost'] 
		                                               + 2*$inRow['avg_unit_cost'] - $rtn['tran_total']);
		//Verify wop
		$wopDbo=CalemFactory::getDbo('wo_part');
		$rows=$wopDbo->fetchBySqlParam('select * from wo_part where wo_id=? and in_id=?', array($this->woAr['id'], $inRow['id']));
		$row=$rows[0];
		...
		$this->assertTrue($row['qty_used']==1 && $row['line_cost']==1*$inRow['avg_unit_cost']);
		//Try out reserved
		if ($this->woAr['status_id'] != 'wos_closed') {
			$reservedDbo=CalemFactory::getDbo('in_reserved');
			$rows=$reservedDbo->fetchBySqlParam('select * from in_reserved where in_id=? and wo_id=?', array($inRow['id'], $this->woAr['id']));
			...
			$this->assertTrue($row['qty']==$this->inReservedAr['qty']-2+$tranReturn['qty']);
		}	                    		
	}
