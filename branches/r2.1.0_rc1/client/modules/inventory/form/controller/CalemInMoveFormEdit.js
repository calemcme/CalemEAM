 ...
 * CalemInMoveFormEdit
 */
function CalemInMoveFormEdit(parent, formId, data) {
...
CalemInMoveFormEdit.prototype = new CalemFormEdit;
CalemInMoveFormEdit.prototype.constructor = CalemInMoveFormEdit;

CalemInMoveFormEdit.prototype.toString = function() { return "CalemInMoveFormEdit";}

/**
 * Initialize data for the transaction
 */
CalemInMoveFormEdit.prototype._createDataModel =
function(data) {
	CalemFormEdit.prototype._createDataModel.call(this);
	this._instantiate(data);
}

CalemInMoveFormEdit.prototype._instantiate =
function(data) {
	...
	rec.getField('orig_loc_id').setRawValue(stockRec.getField('location_id').getRawValue());
	this._stockQty=stockRec.getField('qty').getRawValue();
	rec.getField('qty_orig').setRawValue(stockRec.getField('qty').getRawValue());
...
CalemInMoveFormEdit.prototype._initValidRange =
function() {
	//Enforce qty to check out
	var min=CalemConf['in_tran_conf']['minCheckout'];
	this.setValidRange('qty', min, this._stockQty);
} 

CalemInMoveFormEdit.prototype._loadData =
function() {//Record should have been set already.
	this._cache.bulkLoadCacheForTable(this._modelItem.getId(), this._cacheLoadedCb);
}

CalemInMoveFormEdit.prototype.onCacheLoaded =
function() {
	this.onDataLoaded();
} 

CalemInMoveFormEdit.prototype._render =
function() {
	CalemFormEdit.prototype._render.call(this);
	this._setFieldsReadOnly(['qty_orig', 'orig_loc_id']);
	//Resetting validRange
	this._initValidRange();
}

//Using save function to do transaction
CalemInMoveFormEdit.prototype._onSave =
function(evt) {
	...
	this._inputRow=row;
	rows={in_0: row};
	CalemSoapUtil._onSoapCall('InTranMove', rows, this._inTranCb, null, rows);
}

/**
 * Process soap response from server
 */
CalemInMoveFormEdit.prototype.onInTranCallback =
function(resp, rows) {
...
CalemInMoveFormEdit.prototype._onCancel =
function() {
