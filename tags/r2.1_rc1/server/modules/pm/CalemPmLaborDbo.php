...
class CalemPmLaborDbo extends CalemDbo {	
	...
	public function onDataInserted($id, $baseTable, $baseData, $customTable, $customData) {
		...
		$this->updateLaborHours($baseData['pm_id'], $baseData['hours']);
	}
	...
	public function onDataDeleted($table, $id) {
		...
		$this->updateLaborHours($this->row['pm_id'], $hours);
	}
	...
	public function onDataUpdated($baseTable, $baseCurrent, $baseUpdate, $customTable, $customCurrent, $customUpdate) {
		...
		$this->updateLaborHours($baseCurrent['pm_id'], $hours);		    
	}
	
	public function updateLaborHours($pmId, $hours) {
		//Get pm's DBO
		$pmDbo=CalemFactory::getDbo('pm');
		//Get meter rec.
		$pmRec=$pmDbo->fetchById($pmId);
		$pmRec['labor_hours'] += $hours;
		if ($pmRec['labor_hours']<0) $pmRec['labor_hours']=0;
		$pmDbo->setValue('labor_hours', $pmRec['labor_hours']);
