...
class CalemWoSo extends CalemSoapRequest {
	private $woBo;
	
	public function __construct() {
		parent::__construct();	
		$this->woBo=new CalemWoBo();
	}
	...
	public function gzipIt() {
 	 ...
 	 * Checkout
 	 */
 	public function LookupWoNewByPm() {
 		$param=$this->getParamValue(0);
 		if ($this->logger->isDebugEnabled()) $this->logger->debug("Invoking LookupWoNewByPm, param=" . var_export($param, true));
 		...
 		foreach ($param as $key => $row) {
 			...
 			try { 			
	 			$lookup=$this->woBo->lookupWoNewByPm($tran);
	 			$result[]=array('status'=>0, 'lookup'=>$lookup);
 			} catch (Exception $e) {
