...
/* --------------------------------------------------------------------------*/
    ...
    /**
     * Calem changes: this is the same function as error but accepts an
     * exception
     */
    function error_ex($message, Exception $e, $caller = null)
    {
        $message=$message . ', exception=' . $e->getMessage();
        $this->error($message, $caller);
    }
    ...
    /**
     * Calem changes: same method as warning but accepts an exception
     */
    function warn_ex($message, Exception $e, $caller=null) {
    	$message=$message . ', exception='.$e->getMessage();
    	$this->warn($message, $caller);
    }
